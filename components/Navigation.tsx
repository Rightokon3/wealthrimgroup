          ...
          <div className="flex items-center gap-2">
            ...
            {totalItems > 0 && (
              <Link href="/checkout"
                ...
                {cartStore && (
                  <span className="hidden sm:inline text-orange-100 text-xs">· {cartStore.name}</span>
                )}
              </Link>
            )}
            ...
            {loading ? (
                      ...
                      <div className="p-1.5">
                        {isAdmin && (
                          <Link href="/admin" onClick={() => setUserMenuOpen(false)}
                            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-700 hover:bg-orange-50 hover:text-orange-700 transition-colors">
                        ...
                        {isVendor && (<>
                          ...
                          <Link href="/vendor/products/new" onClick={() => setUserMenuOpen(false)}
                            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-700 hover:bg-orange-50 hover:text-orange-700 transition-colors">
                            <Package className="w-4 h-4" /> Add Product
                          </Link>
                        </>)}
                        {isRider && (
                          <Link href="/rider/dashboard" onClick={() => setUserMenuOpen(false)}
              ...
              <>
                <Link href="/auth/login"
                  className="hidden sm:flex items-center px-4 h-9 rounded-full border border-gray-200 text-gray-700 text-sm font-semibold hover:border-orange-300 hover:bg-orange-50 transition-all">
                  Sign In
                </Link>
                <Link href="/auth/signup"
                  className="flex items-center gap-1.5 px-4 h-9 rounded-full bg-gradient-to-r from-orange-500 to-red-600 text-white text-sm font-bold hover:from-orange-600 hover:to-red-700 shadow-md shadow-orange-200 transition-all">
                  Join Free
                </Link>
              </>
            )}
            ...
            <button onClick={() => setMobileOpen(!mobileOpen)}
              className="md:hidden w-9 h-9 flex items-center justify-center rounded-lg hover:bg-orange-50 transition-colors">
                ...
                <div className="pt-3 border-t border-orange-100 mt-2 space-y-1">
                  {isLoggedIn ? (<>
                    ...
                    <Link href="/auth/login" onClick={() => setMobileOpen(false)}
                      className="block py-2.5 px-3 rounded-lg text-sm font-medium text-gray-700">
                      Sign In
                    </Link>
                    <Link href="/auth/signup" onClick={() => setMobileOpen(false)}
                      className="block py-2.5 px-3 rounded-lg text-sm font-bold text-orange-600 hover:bg-orange-50">
                      Join Free
                    </Link>
                  </>)}
                </div>
