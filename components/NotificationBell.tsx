...
export default function NotificationBell({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    ...
    // Unique suffix per effect run — avoids colliding with a same-named
    // channel from a prior mount whose removeChannel() hasn't finished yet
    // (React Strict Mode double-invokes effects in dev, and removeChannel
    // is async, so reusing a fixed name can hand back an already-subscribed
    // channel object and throw on .on()).
    const uniqueSuffix = Math.random().toString(36).slice(2);
    const channel = supabase
      .channel(`user-notifications-${userId}-${uniqueSuffix}`)
      .on(
        'postgres_changes',
